...
/** ---- Mock: bórralo al conectar tu backend ---- */
async function fakeAsk(_endpoint: string, _q: string) {
  await new Promise((r) => setTimeout(r, 650));
  return {
    answer:
      "Según el manual y las políticas de admisiones, los requisitos incluyen solicitud en línea, DPI/pasaporte, certificación de notas y pago de matrícula. Verifica con Admisiones para tu campus.",
    sources: [
      { id: "s1", title: "Manual de Normas Gráficas — UVG (PDF)", url: "#", type: "pdf", snippet: "Colores institucionales, tipografías y lineamientos para aplicaciones digitales." },
      { id: "s2", title: "Admisiones — UVG Altiplano", url: "#", type: "link", snippet: "Requisitos, proceso de inscripción y contactos." },
    ] as SourceDoc[],
  };
}
/** ------------------------------------------------ */
...
export default function App() {
  ...
  const endpoints = ["/ask", "/ask-nuclia"];
  ...
  try {
    ...
    const assistant: Message = {
      ...
      sources: (res.sources ?? []).map((s: any, i: number) => ({
        ...
        url: s.url ?? undefined,
        type: String(s.url || "").toLowerCase().endsWith(".pdf") ? "pdf" : "link",
    ...
    setMessages((p) => [
      ...p,
      { id: crypto.randomUUID(), role: "assistant", text: `Hubo un problema consultando el backend: ${e.message}` },
    ]);
  } finally {
    setLoading(false);
  }
          ...
          {showLanding ? (
            ...
            >
              ...
              <div className="mx-auto max-w-3xl mt-6 md:mt-8">
                <AskBox onAsk={onAsk} loading={loading} endpoints={endpoints} variant="panel" />
              </div>
            </motion.section>
          ) : (
            <motion.div
      ...
      {!showLanding && (
        <div ref={composerRef} className="fixed inset-x-0 bottom-0 z-30">
          <div className="mx-auto max-w-5xl px-4 pb-[max(0px,env(safe-area-inset-bottom))] pt-3">
            <AskBox onAsk={onAsk} loading={loading} endpoints={endpoints} variant="composer" />
